...
public class UiUtil {
    ...
    public static float getRatio() {
        float ratioWidth = (float) getScreenWidth(BaseApplication.getAppContext()) / 720;
        float ratioHeight = (float) getScreenHeight(BaseApplication.getAppContext()) / 1080;
        return Math.min(ratioWidth, ratioHeight);
    }

    public static void setViewFullWindow(View view) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            view.setPadding(0, UiUtil.getStatusBarHeight(BaseApplication.getAppContext()), 0, 0);
        }
    }

    public static int parseColor(int resColor) {
        return ContextCompat.getColor(BaseApplication.getAppContext(), resColor);
    }
}
